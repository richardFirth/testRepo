Attribute VB_Name = "ZZZ_StringArrayFunctions_5"
'$VERSIONCONTROL
'$*MINOR_VERSION*1.8
'$*DATE*29Jan18
...
' newStrings             | String()        | returns true if all values are found                     |
...
If Not arrayHasStuff(oldSet) Then newStrings = newSet: Exit Function

    For x = LBound(newSet) To UBound(newSet)
        If stringInArray(newSet(x), oldSet) Then
        Else
